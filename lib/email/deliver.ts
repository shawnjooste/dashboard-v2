// The delivery core: sends via Resend, then records the message in
// sent_emails so /communications and the admin activity feed have a
// complete history. Marker-free and alias-free so a plain Node script can
// import it directly — the Supabase client is passed in, never constructed
// here. `send.ts` wraps this for the Next.js app; scripts import it directly.
import type { SupabaseClient } from "@supabase/supabase-js";
import { isSuppressible, splitRecipients } from "./suppression.ts";
import { portalUpdateFooterHtml } from "./portal-update-footer.ts";
...
// Not exported: `send.ts` re-declares its own copy for external callers, so
// this module's only runtime export stays `deliverEmail` (see Step 8 of the
// brief, which checks `Object.keys(m)` on a plain-Node import).
const DEFAULT_FROM = '"Rocking" <no-reply@send.rocking.one>';

/** Sends the email and records it. `sb` is the Supabase client to use (a
 *  service-role client in production; a stub in tests) — callers own its
 *  construction so this module never imports Next.js server-only code.
 *  Returns Resend's raw message id (callers that need a threading header
 *  format it themselves), or null when the send was skipped for want of an
 *  API key. Throws only if Resend rejects the send; a logging failure never
 *  propagates. */
export async function deliverEmail(
  ...
  if (isSuppressible(opts.category)) {
    ...
    const { data: outRows } = await sb
      ...
      .in("email", lowered);
    const optedOut = new Set((outRows ?? []).map((r: { email: string }) => r.email.trim().toLowerCase()));
